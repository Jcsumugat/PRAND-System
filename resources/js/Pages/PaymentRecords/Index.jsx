...
export default function Index({ payments, filters }) {
    ...
    const handleSearch = (e) => {
        e.preventDefault();
        router.get(route('payments.index'), { search, type, method }, { preserveState: true });
    };

    const handleDelete = (id, receiptNumber) => {
        if (confirm(`Are you sure you want to delete payment record ${receiptNumber}?`)) {
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-gradient-to-r from-green-200 to-emerald-200">
                                <tr>
                                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase">Receipt #</th>
                                    ...
                                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase">Payment Date</th>
                                    ...
                                    <th className="px-6 py-4 text-left text-xs font-bold text-gray-700 uppercase">Received By</th>
                                    ...
                                    payments.data.map((payment) => (
                                        <tr key={payment.id} className="hover:bg-green-50 transition">
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <span className="text-sm font-semibold text-green-700">{payment.receipt_number}</span>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="text-sm font-medium text-gray-900">
                                                    {payment.deceased_record.fullname}
                                                </div>
                                                <div className="text-sm text-gray-500">
                                                    Tomb: {payment.deceased_record.tomb_number}
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <span className="text-sm font-bold text-gray-900">₱{parseFloat(payment.amount).toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getTypeBadge(payment.payment_type)}`}>
                        ...
                        <div className="bg-gray-50 px-6 py-4 border-t border-gray-200">
                            <div className="flex items-center justify-between">
                                <div className="text-sm text-gray-700">
                                    ...
                                    <span className="font-semibold">{payments.total}</span> results
                                </div>
